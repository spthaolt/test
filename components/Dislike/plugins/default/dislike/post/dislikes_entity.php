...
$count = $dislikes->CountDisLikes($params['entity_guid'], 'entity');
$user_liked = '';
if (ossn_isLoggedIn()) { 
            if ($dislikes->isDisliked($params['entity_guid'], ossn_loggedin_user()->guid, 'entity')) {
                $user_disliked = true;
            }
}
...
if ($count) { ?>
    <div class="like-share dislike-share">
        ...
        <?php
            ...
            $link['onclick'] = "Ossn.ViewDisLikes({$params['entity_guid']}, 'entity');";
            $link['href'] = '#';
