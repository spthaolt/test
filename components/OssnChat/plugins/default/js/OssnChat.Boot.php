...
if(OssnChat['allfriends']){
  $.each(OssnChat['allfriends'], function(key, data){
       ...
       if($item.length){
        ...
        if($item.length == 0){
         var appendata = '<div data-toggle="tooltip" title="'+data['name']+'" class="friends-list-item" id="friend-list-item-'+data['guid']+'" onClick="Ossn.ChatnewTab('+data['guid']+');"><div class="friends-item-inner"><div class="icon"><img class="ustatus" src="'+data['icon']+'" /></div><div class="name">'+data['name']+'</div></div></div>';    
